...
nation %<>% 
  mutate(lag_cases = as.double(lag(cases)), 
         relative = cases_delta / lag_cases, 
         cases_delta_ma = movingAverage(cases_delta),
         cases_perc = case_when(
           cases_perc == Inf ~ 100,
           TRUE ~ (100 * sum(cases_delta) / lag_cases)), 
         cases_dir = (cases_delta_ma < lag(cases_delta_ma)),
         cases_perc_ma = movingAverage(cases_perc),
         lag_deaths = as.double(lag(deaths)), 
         relative = deaths_delta / lag_deaths, 
         deaths_delta_ma = movingAverage(deaths_delta),
...
nation_average <- nation %>% 
  mutate(
    week_num = epiweek(date),
    year_num = epiyear(date),
    cases_perc_2 = 100 * (cases_delta_ma-lag(cases_delta_ma))/lag(cases_delta_ma),
    cases_delta_ma = round(cases_delta_ma, 0),
    deaths_delta_ma = round(deaths_delta_ma, 0)
    #cases_perc_3 = 
  ) %>% 
  select(week_num, year_num, date, cases_delta_ma, deaths_delta_ma) %>% 
...
nation_fourteen_days <- nation_average %>% 
  mutate(
    week_group = week_num - week_num %% 2 + 1 + 53 * (year_num - 2020),
  ) %>% 
  group_by(year_num, week_group) %>% 
  summarize(
    two_week_cases_delta = sum(cases_delta_ma),
    two_week_deaths_delta = sum(deaths_delta_ma)
  ) %>%
  ungroup() %>% 
  mutate(
    cases_perc_change = 100 * (two_week_cases_delta-lag(two_week_cases_delta))/lag(two_week_cases_delta),
    deaths_perc_change = 100 * (two_week_deaths_delta-lag(two_week_deaths_delta))/lag(two_week_deaths_delta)
  )
...
100 * (nation_average[59,4] - nation_average[58,4]) / nation_average[58,4]
nation %>% filter(week(nation$date) %in% c(8,9,10), year(date) == 2021) %>% select(date, cases_delta_ma)
plot(nation_average$deaths_delta_ma ~ nation_average$max_date)
```
...
nation_today_cases <- nation_today %>% 
  mutate(type="Confirmed Cases", 
         cases_per_hundred_thousand = round(cases_per_hundred_thousand, 2), 
         "2 Week Change" = round(rev(nation_fourteen_days$cases_perc_change)[1],2)
  ) %>% 
  select(type, date, cases, cases_delta, cases_per_hundred_thousand, "2 Week Change") %>% 
  rename(Date = date, "Total" = cases, "24 Hour Change" = cases_delta, "Total Per 100,000"= cases_per_hundred_thousand, "Type" = type)

nation_today_deaths <- nation_today %>% 
  mutate(type="Confirmed Deaths", 
         deaths_per_hundred_thousand = round(deaths_per_hundred_thousand, 2), 
         "2 Week Change" = round(rev(nation_fourteen_days$deaths_perc_change)[1],2)
  ) %>%
  select(type, date, deaths, deaths_delta, deaths_per_hundred_thousand, "2 Week Change") %>% 
  rename(Date = date, "Total" = deaths, "24 Hour Change" = deaths_delta, "Total Per 100,000"= deaths_per_hundred_thousand, "Type" = type)
...
Ratios
```{r, warning=F}
nation_percs <- nation_last30 %>% 
  mutate(uninfected = population - cases - deaths) %>% 
  select(date, population, cases, deaths, uninfected) %>% 
  pivot_longer(cols=c(uninfected, cases, deaths), names_to="type", values_to="count") %>% 
  mutate(type=factor(type, levels=c("uninfected", "cases", "deaths"))) %>% 
  mutate(type=fct_infreq(type))
nat_p_ratios <- nation_percs %>% ggplot(aes(x=date, y=count, fill=type, color=type))+
  #geom_line(size=1.1)+ 
   geom_area(alpha = 0.5)+
  labs(title = "Ratios", x = "Date")+
  jhu_caption+
    theme(aspect.ratio = 6/14,
  ...
  scale_y_continuous(breaks = breaks_extended(n=7),
                     ...
                     name = "Total")+
                     #sec.axis = dup_axis(),
                     #sec.axis=sec_axis(trans=~./(nation_percs$population) * 100000, name = "Per Hundred Thousand People"))+
  scale_x_date(date_breaks = "8 weeks", date_labels = "%b %d")
 
  #geom_hline(yintercept=max(nation_percs$deaths), color="red")+
  #geom_hline(yintercept=nation_percs$deaths[nrow(nation_percs)-14], color="green")+pq
nat_p_ratios
```
